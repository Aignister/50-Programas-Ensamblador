...
// Programa que encuentra el minimo en un arreglo
...
.section .data
    ...
    result_msg: .ascii "Minimo: "       // Mensaje para el resultado
...
_start:
    ...
    // Cargar la dirección del arreglo y el primer valor como mínimo inicial
    adr x2, array                       // x2 apunta al inicio del arreglo
    ldr w3, [x2], #4                    // Cargar el primer valor como mínimo inicial y avanzar

find_min_loop:
    subs w1, w1, #1                     // Decrementar el contador de elementos
    beq print_min                       // Si no hay más elementos, imprimir el mínimo

    ldr w4, [x2], #4                    // Cargar el siguiente valor del arreglo y avanzar
    cmp w4, w3                          // Comparar el valor actual con el mínimo
    csel w3, w4, w3, lt                 // Si el valor actual es menor, actualizar el mínimo
    b find_min_loop                     // Repetir el ciclo

print_min:
    // Imprimir el mensaje "Minimo: "
    ...
    // Convertir el valor mínimo a ASCII
